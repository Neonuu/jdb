      ...
      success:function(data){
        ...
        for(var i=0;i<data.list.length;i++){
          str+="<tr><td>"+data.list[i].name+"</td><td>"+data.list[i].order_number+"</td><td>"+
                ...
                data.list[i].province+"</td><td>"+data.list[i].city+"</td><td>"+data.list[i].add_time+"</td><td>"
                +data.list[i].auditer_name+"</td><td><a href='javascript:seeRemarks("+
                data.list[i].remarks+");'  class='btn btn-sm'>查看备注</a></td></tr>";
        }
        $("#tbody1").append(str);
        $("#btn2").html("第"+data.page);
        $("#span").html(data.totalCount);
...
$('#province').on('change', function(){
	...
	$.ajax({
     ...
     $("#btn28").attr("class","btn btn-sm");
   
});

function seeRemarks(remarks){
	layer.open({
	    type: 0,
   ...
   <div class="templatemo-content-wrapper">
        <div class="faq-query">
          <div class="item">
			     ...
			     <input id="date1" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="订单关闭时间" style="width:180px;"/>
				 &nbsp;&nbsp;&nbsp;&nbsp;至&nbsp;&nbsp;&nbsp;&nbsp;
				 <input id="date2" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'}) "  placeholder="订单关闭时间" style="width:180px;"/>
			     <br/><br/>
			      &nbsp;&nbsp;
               ...
               <table border='1px' id="table1" class="table table-striped table-bordered">
				    <thead>
			           <tr>
				           ...
				           <td style="width:115px;">关闭时间</td><td style="width:85px;">操作者</td><td style="width:100px;">备注</td>
			           </tr>
		            </thead>
