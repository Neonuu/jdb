...
$("#btn1").click(function(){
    ...
    $.ajax({
      ...
      data:{"page":page,"user_phoneNumber":$("#name").val(),"order_number":$("#order_number").val(),
            ...
            "city":$("#city").val(),"date1":$("#date1").val(),"date2":$("#date2").val()},
      dataType:"json",
      success:function(data){
        ...
        for(var i=0;i<data.list.length;i++){
          if(data.list[i].status_cailiao==1){
              ...
              data.list[i].province+"</td><td>"+data.list[i].city+"</td><td>"+data.list[i].add_time+"</td><td><a href='javascript:getOrderId("+
              data.list[i].ID+")'  class='btn btn-sm'>通过</a>&nbsp;<a href='javascript:getOrderIdNo("+
              data.list[i].ID+")'  class='btn btn-sm'>关闭</a>&nbsp;<a href='javascript:chaKanXiangQing("+
              data.list[i].ID+")'  class='btn btn-sm'>查看详情</a></td></tr>";
          }else{
        	  str+="<tr><td>"+data.list[i].name+"</td><td>"+data.list[i].order_number+"</td><td>"+
              data.list[i].user_identity+"</td><td>"+data.list[i].product+"</td><td>"+data.list[i].true_name+
              "</td><td>"+data.list[i].user_idCardNumber+"</td><td>"+data.list[i].tel_number+"</td><td>"+
              data.list[i].province+"</td><td>"+data.list[i].city+"</td><td>"+data.list[i].add_time+"</td><td><a href='javascript:buChongCaiLiao("+
              data.list[i].ID+","+data.list[i].user_id+")'  class='btn btn-sm'>补充材料</a>&nbsp;<a href='javascript:getOrderIdNo("+
              data.list[i].ID+")'  class='btn btn-sm'>关闭</a></td></tr>";
          }
 	 ...
 	 $.ajax({
 		 ...
 		 success:function(data){
 			 if(data.err){
 				...
 				layer.msg("提交成功");
 				setTimeout(function(){
 					window.location.href="toOrderOneIndex.do"; 
 				},1000);			 
 			 }
 		 }
 	 });
...
function getOrderIdNo(orderId){
	layer.prompt({
	    title: '请输入关闭订单的原因：',
	    formType:2 //prompt风格，支持0-2
		}, function(remarks){
			 $.ajax({
		 		 url:"noOrderOneIndex.do",
		 		 data:{"id":orderId,"remarks":remarks},
		 		 type:"post",
		 		 dataType:"json",
		 		 success:function(data){
		 			 if(data.err){
...
//补充材料调用的函数
function buChongCaiLiao(ID,user_id){
	//alert(user_id);
	window.location.href="toBuChongCaiLiao.do?ID="+ID+"&user_id="+user_id; 
}
//查看订单详情调用的函数
   ...
   <div class="templatemo-content-wrapper">
        <div class="faq-query">
          <div class="item">
			     ...
			     <input id="date1" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="订单时间" style="width:180px;"/>
				 &nbsp;&nbsp;&nbsp;&nbsp;至&nbsp;&nbsp;&nbsp;&nbsp;
				 <input id="date2" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'}) "  placeholder="订单时间" style="width:180px;"/>
			     <br/><br/>
			      &nbsp;&nbsp;
        ...
        <div class="widget-table"  >
            <div class="widget-content" >
               <table border='1px' id="table1" class="table table-striped table-bordered">
				    <thead>
			           <tr>
				           <td style="width:80px;">客户经理电话</td><td style="width:125px;">订单号</td><td style="width:70px;">客户身份</td>
				           <td style="width:120px;">产品名</td><td style="width:80px;">客户名</td><td style="width:169px;">身份证号</td>
				           <td style="width:100px;">电话号</td><td style="width:70px;">省份</td><td style="width:60px;">城市</td>
				           <td style="width:85px;">下单时间</td><td style="width:160px;">操作</td>
			           </tr>
		            </thead>
