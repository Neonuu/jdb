      ...
      success:function(data){
        ...
        for(var i=0;i<data.list.length;i++){
          str+="<tr><td>"+data.list[i].name+"</td><td>"+data.list[i].order_number+"</td><td>"+
                ...
                data.list[i].province+"</td><td>"+data.list[i].city+"</td><td>"+data.list[i].add_time+"</td><td>"+data.list[i].auditer_name+"</td><td><a href='javascript:getOrderId("+
...
function getOrderId(orderId){
   if(confirm("您确定已放款成功吗？")){
 	 $.ajax({
 		 url:"okOrderThreeIndex.do",
 		 data:{"id":orderId},
 		 type:"post",
 		 dataType:"json",
 		 success:function(data){
 			 if(data.err){
 				...
 				layer.msg("提交成功");
 				setTimeout(function(){
 					 window.location.href="toOrderThreeIndex.do"; 
 				},1000);	 				
 			 }
 		 }
 	 });
...
function chaKanXiangQing(ID){
	layer.open({
	    ...
	    shade: 0.8,
	    area: ['1000px', '95%'],
	    content: 'toChaKanXIangQingLock.do?ID='+ID //iframe的url
	}); 
}
</script>
</head>
   ...
   <div class="templatemo-content-wrapper">
        <div class="faq-query">
          <div class="item">
			     ...
			     <input id="date1" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="批款时间" style="width:180px;"/>
				 &nbsp;&nbsp;&nbsp;&nbsp;至&nbsp;&nbsp;&nbsp;&nbsp;
				 <input id="date2" class="Wdate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'}) "  placeholder="批款时间" style="width:180px;"/>
			     <br/><br/>
			      &nbsp;&nbsp;
               ...
               <table border='1px' id="table1" class="table table-striped table-bordered">
				    <thead>
			           <tr>
				           ...
				           <td style="width:115px;">批款时间</td><td style="width:85px;">操作者</td><td style="width:160px;">操作</td>
			           </tr>
		            </thead>
