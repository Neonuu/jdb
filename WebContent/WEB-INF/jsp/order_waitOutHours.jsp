      ...
      success:function(data){
        ...
        for(var i=0;i<data.list.length;i++){
          str+="<tr><td>"+data.list[i].name+"</td><td>"+data.list[i].order_number+"</td><td>"+
                ...
                data.list[i].ID+")'  class='btn btn-sm'>拒绝</a></td></tr>";
        }
        $("#tbody1").append(str);
        $("#btn2").html("第"+data.page);
        $("#span").html(data.totalCount);
...
function getOrderId(orderId){
	if(confirm("您确定通过信息审核吗？")){
 	 $.ajax({
 		 url:"xinShenOutHoursOrderIndex.do",
 		 data:{"id":orderId},
 		 type:"post",
 		 dataType:"json",
 		 success:function(data){
 			 if(data.err){
 				 ...
 				 alert("提交成功");
 				 window.location.href="toOutHoursOrder.do"; 
 			 }
 		 }
 	 });
	 }
	...
	function getOrderIdNo(orderId){
		if(confirm("您确定不通过信息审核吗？")){
	 	 $.ajax({
	 		 url:"noXinShenOutHoursOrderIndex.do",
	 		 data:{"id":orderId},
	 		 type:"post",
	 		 dataType:"json",
	 		 success:function(data){
...
<body>
    <div style="text-align: left;">
	 ...
	 <div style="text-align: left;" >
	 <input style="width:200px" type="button" value="锁定订单"  onclick="window.location.href='toInHoursOrder.do'"/>
	 <input style="width:200px" type="button" value="非锁定订单"  class="btn bg-warning" />
	 <hr/>
	</div> 
        ...
        <div class="widget-table"  >
            <div class="widget-content" >
               <table border='1px' id="table1" class="table table-striped table-bordered">
				    <thead>
			           <tr>
				           <td style="width:80px;">客户经理电话</td><td style="width:150px;">订单号</td><td style="width:70px;">用户身份</td>
				           <td style="width:120px;">产品名</td><td style="width:100px;">客户名</td><td style="width:180px;">身份证号</td>
				           <td style="width:100px;">电话号</td><td style="width:70px;">省份</td><td style="width:80px;">城市</td>
				           <td style="width:85px;">下单时间</td><td style="width:115px;">操作</td>
			           </tr>
		           </thead>
