...
function getOrderId(orderId){
   if(confirm("您确定通过信息审核吗？")){
 	 $.ajax({
 		 url:"xinShenInHoursOrderIndex.do",
 		 data:{"id":orderId},
 		 type:"post",
 		 dataType:"json",
 		 success:function(data){
 			 if(data.err){
 				 ...
 				 alert("提交成功");
 				 window.location.href="toInHoursOrder.do"; 
 			 }
 		 }
 	 });
	}
...
function getOrderIdNo(orderId){
	   if(confirm("您确定不通过信息审核吗？")){
	 	 $.ajax({
	 		 url:"noXinShenInHoursOrderIndex.do",
	 		 data:{"id":orderId},
	 		 type:"post",
	 		 dataType:"json",
	 		 success:function(data){
...
<body>
   <div style="text-align: left;" >
	...
	<div style="text-align: left;" >
	 <input style="width:200px" type="button" value="锁定订单" class="btn bg-warning" />
	 <input style="width:200px" type="button" value="非锁定订单"  onclick="window.location.href='toOutHoursOrder.do'" />
	 <hr/>
	</div> 
