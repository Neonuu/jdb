...
$("#btn1").click(function(){
    ...
    $("#tbody1").html("");
    $.ajax({
      url:"dataAuditingOrder.do",
      type:"post",
      data:{"page":page,"name":$("#name").val(),"order_number":$("#order_number").val(),"user_idCardNumber":$("#idCardNumber").val(),
            "product":$("#product").val(),"date1":$("#date1").val(),"date2":$("#date2").val()},
      dataType:"json",
      success:function(data){
        ...
        for(var i=0;i<data.list.length;i++){
            ...
            data.list[i].user_identity+"</td><td>"+data.list[i].product+"</td><td>"+data.list[i].true_name+"</td><td>"+data.list[i].user_idCardNumber+
            "</td><td>"+data.list[i].tel_number+"</td><td>"+data.list[i].province+"</td><td>"+data.list[i].city+"</td><td>"+data.list[i].add_time+
            "</td><td>"+data.list[i].recommemd+"</td><td><a href='javascript:getOrderId("+
...
function getOrderIdNo(orderId){
	   if(confirm("您确定不通过审核吗？")){
	 	 $.ajax({
	 		 url:"shenHeFailIndex.do",
	 		 data:{"id":orderId},
	 		 type:"post",
	 		 dataType:"json",
	 		 success:function(data){
        ...
        <div class="faq-query">
          <div class="item">
		    ...
		    <!--  <input id="product" placeholder="产品名" style="width:180px;"/> -->
			 ...
			 <input id="date1" class="Wdate" type="text" style="width:180px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="信审时间"/>&nbsp;至&nbsp;
			 <input id="date2" class="Wdate" type="text" style="width:180px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'}) "  placeholder="信审时间"/>&nbsp;&nbsp;
        ...
        <div class="widget-table">
            <div class="widget-content" style="height: 400px;">
               <table border='1px' id="table1" class="table table-striped table-bordered">
				   <thead>
			           <tr>
				           <td>客户经理电话</td><td style="width:150px;">订单号</td><td>客户身份</td><td>产品名</td><td>客户名</td><td style="width:169px;">身份证号</td>
				           <td>电话号</td><td style="width:70px;">省份</td><td style="width:60px;">城市</td><td style="width:80px;">信审时间</td>
				           <td style="width:60px;">信审人</td><td style="width:100px;">操作</td>
			           </tr>
		           </thead>
