...
<form name="searchForm" method="post" action="/deleteAdmin.do">
...
<table class="table table-striped table-bordered" >
    ...
    <c:forEach items="${alist}" var="a">
    	<tr>
	        ...
	        <c:otherwise>
	        <c:if test="${admin == 1}">
	        	<a class="btn btn-sm btn-gray" href="/deleteAdminDEL.do?user_id=${a.user_id}" onclick="javascript:return p_del()">删除</a>
	        </c:if>	
	        <c:if test="${admin == 2}">
	        	<c:if test="${a.admin == 3}">
	        	<a class="btn btn-sm btn-gray" href="/deleteAdminDEL.do?user_id=${a.user_id}" onclick="javascript:return p_del()">删除</a>
